/*
  ...
  1. Changes
    - Insert test news articles into the news table
    - Set published status to true
    - Include sample images and content
*/
...
(
  ...
  '学院新闻',
  '[{"url": "https://images.unsplash.com/photo-1516321318423-f06f85e504b3", "name": "scholarship"}]'::jsonb,
  true,
  now(),
  now(),
...
(
  ...
  '学院新闻',
  '[{"url": "https://images.unsplash.com/photo-1507413245164-6160d8298b31", "name": "exchange"}]'::jsonb,
  true,
  now(),
  now(),
  (SELECT id FROM users WHERE email = 'admin@jlu.edu.cn' LIMIT 1)
...
(
  ...
  '合作交流',
  '[{"url": "https://images.unsplash.com/photo-1581091226825-a6a2a5aee158", "name": "meeting"}]'::jsonb,
  true,
  now(),
  now(),
  (SELECT id FROM users WHERE email = 'admin@jlu.edu.cn' LIMIT 1)
...
(
  ...
  '学生活动',
  '[{"url": "https://images.unsplash.com/photo-1562774053-701939374585", "name": "class-meeting"}]'::jsonb,
  true,
  now(),
  now(),
  (SELECT id FROM users WHERE email = 'admin@jlu.edu.cn' LIMIT 1)
